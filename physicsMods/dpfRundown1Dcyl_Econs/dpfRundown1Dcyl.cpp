 ...
 * (1D railgun) in cyl coords
 *
 * dN/dt  + d(r*Mr)/r/dr = 0
 * dM/dt  + d(r*(Mr*Vr + P))/r/dr - P/r = -Jz*By
 * dE/dt  + d(r*(E+P)*Vr)/r/dr = Jz*Ez 
 * dBy/dt + d(-Ez)/dr = 0
 * delta0*dEz/dt + d(-r*By)/r/dr = -Jz
 * epsilon0*dJz/dt = [Ez + Vr*By] - eta*Jz
 *
 * Vr = Mr/N;
 * E = 0.5*N*Vr^2 + P/(gamma-1);
 * Jz = [Ez + Ur*By]/eta
 * P = (E - 0.5*N*Vr^2 - By^2/2)*(gamma-1)
 *
***/
...
void Physics::advance(const domainGrid& Xgrid, const double dt)
{
   ...
   for (auto n=1; n<Nsub+1; n++) {
	 ...
	 N.at(i) = Nold.at(i) - thisdt*(FluxN.at(i)-FluxN.at(i-1))/rcc.at(i)/Xgrid.dX;
		 ...
		 + thisdt*(P.at(i)/rcc.at(i) - Jzcc.at(i)*B.at(i));
         E.at(i) = Eold.at(i) - thisdt*(FluxE.at(i)-FluxE.at(i-1))/rcc.at(i)/Xgrid.dX
   ...
   for (auto i=nXg-1; i<nMax-nXg+1; i++) {
      ...
      //
      Ez.at(i) = -(FluxEz.at(i+1)-FluxEz.at(i))/rce.at(i)/Xgrid.dX + d0*Ezold.at(i)
...
void computeFluxes(const domainGrid& Xgrid, const int order)
{
   ...
   FluxNcc = rcc*M;
   FluxMcc = rcc*(M*V + P);
   FluxEcc = rcc*(0.5*V*M + P*gamma0/(gamma0-1.0) )*V;
   //FluxScc = V*S;
   FluxB0cc = V*B;
   FluxEz = -rcc*B;
   ...
   */
   ...
   Jz0 = Jz0/rce;
   if(procID==0) {
      setXminBoundary(Jz0,2.0*B.at(2)/rcc.at(2),0.0);
   } 
