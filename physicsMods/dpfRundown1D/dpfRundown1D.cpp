/***
 * 
 * physics module for 1D ideal MHD shock
 * (1D railgun)
 *
 * dN/dt  + d(Mx)/dx = 0
 * dMx/dt + d(Mx*Ux + P + Bz^2/2)/dx = 0
 * de/dt  + d((e+P+Bz^2/2)*Ux)/dx = 0  
 * dBz/dt + d(Ux*Bz)/dx
 *
***/
...
double eta0=0.001;     // resistivity
double delta0=0.01;    // relaxation const (v/c)^2
double B0 = 0.0, E0; // boundary value of magnetic field
int Nsub;             // time-solver subcycle steps
vector<double> N, M, S, B, Ez;   // time-evolving variables
vector<double> Cs, V, P, J, J0; // derived variables
vector<double> Nold, Mold, Sold, Bold, Ezold;
...
void Physics::advance(const domainGrid& Xgrid, const double dt)
{
   ...
   for (auto n=1; n<Nsub+1; n++) {
      ...
      // First update explicit terms from n to n+1
      for (auto i=nXg; i<nMax-nXg; i++) {
		 ...
		 - thisdt*J.at(i)*Bold.at(i);
         S.at(i) = Sold.at(i) - thisdt*(FluxS.at(i)-FluxS.at(i-1))/Xgrid.dX
		 + thisdt*(gamma0-1.0)*eta0*pow(Nold.at(i),1.0-gamma0)*J.at(i)*J.at(i);
      ...
      //vector<double> FluxEzold = FluxEz;
      //FluxEz = (FluxEz+FluxEzold)/2.0;

   } // finish subcycle steps for N, M, S, and B

   // update implicit variables
   for (auto i=nXg; i<nMax-nXg; i++) {
      Ez.at(i) = Ezold.at(i) - thisdt/delta0*(FluxEz.at(i)-FluxEz.at(i-1))/Xgrid.dX
		  - thisdt/delta0*M.at(i)/N.at(i)*B.at(i)/eta0;
      Ez.at(i) = Ez.at(i)/(1.0+thisdt/delta0/eta0);
   }
   
   // update Ez at lower boundary for Bfield flux
   E0 = eta0*(B.at(2)-B0)/Xgrid.dX - (V.at(1)+V.at(2))*(B0+B.at(2))/4.0;
   //E0 = eta0*(B.at(2)-B0)/Xgrid.dX;
   //double exparg = thisdt/eta0;
   //E0 = E0*exp(-exparg) + eta0*(1.0-exp(-exparg))*(B.at(2)-B0)/Xgrid.dX;
   if(procID==0) {
      setXminBoundary(Ez, E0, 0.0);
   }
   if(procID==numProcs-1) {
...
void computeFluxes(const domainGrid& Xgrid, const int order)
{
   ...
   MPI_Comm_rank (MPI_COMM_WORLD, &procID);
   MPI_Comm_size (MPI_COMM_WORLD, &numProcs);
   vector<double> Cspeed, FluxNcc, FluxMcc, FluxScc, FluxBcc, FluxEzcc;
   vector<double> Cspeed2, Cvaceff, FluxP; 
   FluxNcc.assign(nCC,0.0);
   FluxMcc.assign(nCC,0.0);
   FluxScc.assign(nCC,0.0);
   FluxBcc.assign(nCC,0.0);
   FluxEzcc.assign(nCC,0.0);
   ...
   //E0 = eta0*(B.at(2)-B0)/Xgrid.dX - (V.at(1)+V.at(2))*(B0+B.at(2))/4.0;
   //E0 = eta0*(B.at(2)-B0)/Xgrid.dX;
   double exparg = deltat/eta0;
   E0 = E0*exp(-exparg) + eta0*(1-exp(-exparg))*(B.at(2)-B0)/Xgrid.dX;
   if(procID==0) {
      setXminBoundary(Ez, E0, 0.0);
   }
   if(procID==numProcs-1) {
   ...
   */ 
   ...
   if(advScheme0 == "TVD") {
      ...
      Xgrid.computeFluxTVD(FluxB,FluxL,FluxR,FluxRatio,FluxLim,
                           FluxBcc,Cspeed,B,Nsub);
      //Xgrid.computeFluxTVD(FluxEz,FluxL,FluxR,FluxRatio,FluxLim,
      //                     FluxEzcc,Cspeed,Ez,Nsub);
   ...
   if(procID==0) {
      ...
      //setXminBoundary(FluxB, -E0, 0.0);
      //setXminBoundary(FluxEz, -(B0+B.at(2))/2.0, 0.0);   
   }   
