...
vector<double> N, M, E, B;   // time-evolving variables
vector<double> Cs, V, P; // derived variables
vector<double> Nold, Mold, Eold, Bold;
vector<double> FluxRatio, FluxLim;
vector<double> FluxR, FluxL;  // flux at cell-edges   
vector<double> FluxN, FluxM, FluxE, FluxB;
                      ...
                      HDF5dataFile& dataFile)
{
   ...
   if(Nvar.isObject()) { 
      ...
      if(procID==numProcs-1) setXmaxBoundary(N, 1.0);   
      Xgrid.communicate(N);
      Nold  = N;
   ...
   if(Pvar.isObject()) { 
      ...
      if(procID==numProcs-1) setXmaxBoundary(P, 1.0);   
      Xgrid.communicate(P);
   ...
   if(Bvar.isObject()) { 
      ...
      //B = (1.0-B)/2.0;   
      //if(procID==0) setXminBoundary(B, 0.0, 1.0);   
      if(procID==numProcs-1) setXmaxBoundary(B, 0.0);
      Xgrid.communicate(B);
      Bold = B;
   } else {
   ...
   E = P/(gamma0-1.0) + 0.5*M*V + 0.5*B*B;
   Eold  = E;
   Cs = sqrt(gamma0*P/N);
...
void Physics::advance(const domainGrid& Xgrid, const double dt)
{
   ...
   for (auto n=1; n<Nsub+1; n++) {
      ...
      if(procID==0) {
         ...
         //setXminBoundary(E, 1.0/(gamma0-1.0)+0.5, 0.0);   
         //setXminBoundary(B, 0.0, 1.0);   
      }
      if(procID==numProcs-1) {
         setXmaxBoundary(N, 1.0);   
         setXmaxBoundary(M, 0.0);   
         setXmaxBoundary(E, 1.0/(gamma0-1.0));   
         setXmaxBoundary(B, 0.0);   
      }
      Xgrid.communicate(N);
      Xgrid.communicate(M);
      Xgrid.communicate(E);
...
void computeFluxes(const domainGrid& Xgrid)
{
   ...
   MPI_Comm_rank (MPI_COMM_WORLD, &procID);
   MPI_Comm_size (MPI_COMM_WORLD, &numProcs);
   vector<double> Cspeed, FluxNcc, FluxMcc, FluxEcc, FluxBcc;
   vector<double> FluxP; 
   ...
   //
   V  = M/N;
   P  = (E-0.5*M*M/N-0.5*B*B)*(gamma0-1);
   Cs = sqrt(gamma0*P/N);
   //if(procID==0) setXminBoundary(V, 0.0);   
   //if(procID==0) setXminBoundary(Cs, 0.0);   
   ...
   FluxNcc = M;
   FluxMcc = M*M/N + P + B*B/2.0;
   FluxEcc = V*(E + P + B*B/2.0);
   FluxBcc = V*B;
   ...
   if(advScheme0 == "TVD") {
      ...
      Xgrid.InterpToCellEdges(FluxB,FluxBcc,B,advScheme0);
   } 
