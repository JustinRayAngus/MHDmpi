...
double mM;          // me/Mi
double tauiMax;     // maximum ion collision time for  viscosity coefficient
...
// viscousity terms
//
matrix2D<double> Pii_xxold, Pii_xx0, Pii_xx, Pii_xz, Pii_zx, Pii_yy, Pii_zz;
matrix2D<double> divPii_x, divPii_z, divUidotPii;
matrix2D<double> etaVis, etaVis_x, etaVis_z, UidotPii_x, UidotPii_z;
...
void computeStressTensor(const domainGrid&);
                             ...
                             const double thisdt, const double thist)
{
   ...
   for (auto j=nZg; j<nZcc-nZg; j++) {
      for (auto i=nXg-1; i<nXcc-nXg; i++) {
         ...
         //
         d0Piixx = thisdt/delta0*N_x(i,j)/etaVis_x(i,j);
         Pii_xx(i,j)  = (Pii_xxold(i,j) + d0Piixx*Pii_xx0(i,j))/(1.0 + d0Piixx);
      }
   }
   //cout << "JRA: max(Pii_xx) = " << max(Pii_xx) << endl;
   //cout << "JRA: max(etaVis_x) = " << max(etaVis_x) << endl;
   //cout << "JRA: d0Piixx = " << d0Piixx << endl;
...
void computeStressTensor( const domainGrid&  Xgrid )
{
   ...
   // set viscousity coefficient
   //
   //taui = max(taui,tauiMax); // upper threshold to collision time for visc
   etaVis = 0.96*Pi*min(taui,tauiMax);
   Xgrid.InterpToCellEdges(etaVis_x,etaVis,etaVis,"C2",0);
   Xgrid.InterpToCellEdges(etaVis_z,etaVis,etaVis,"C2",1);
   Xgrid.communicate(etaVis_x);

   
   // compute Vx derivatives
   //
   matrix2D<double> dVxdx_x, dVxdz_z, dVxdz, dVxdz_x;
   dVxdx_x.initialize(nXce,nZcc,0.0); 
   dVxdz_z.initialize(nXcc,nZce,0.0); 
   dVxdz.initialize(nXcc,nZcc,0.0); 
   dVxdz_x.initialize(nXce,nZcc,0.0); 
   //
   Xgrid.DDX(dVxdx_x,Vx);
   Xgrid.DDZ(dVxdz_z,Vx);
   Xgrid.DDZ(dVxdz,Vx);
   Xgrid.InterpToCellEdges(dVxdz_x,dVxdz,dVxdz,"C2",0);


   // compute Vz derivatives
   //
   matrix2D<double> dVzdz_z, dVzdx_x, dVzdx, dVzdx_z;
   dVzdz_z.initialize(nXcc,nZce,0.0); 
   dVzdx_x.initialize(nXce,nZcc,0.0); 
   dVzdx.initialize(nXcc,nZcc,0.0); 
   dVzdx_z.initialize(nXcc,nZce,0.0); 
   //
   Xgrid.DDZ(dVzdz_z,Vz);
   Xgrid.DDX(dVzdx_x,Vz);
   Xgrid.DDX(dVzdx,Vz);
   Xgrid.InterpToCellEdges(dVzdx_z,dVzdx,dVzdx,"C2",1);
   ...
   Xgrid.DDX(divUx_x,hy_cc*Vx);
   divUx_x /= hy_ce;
   vector<double> divUx0;
   divUx0.assign(nZcc,0.0);
   if(procID==0 && geometry0=="CYL" && XlowBC.compare("axis") == 0) {
      for (auto j=0; j<nZcc; j++) {
         divUx0.at(j) = 2.0*Vx(nXg,j)/hy_cc(nXg,j);
      }
   ...
   //
   Pii_xx0 = -etaVis_x*(2.0*dVxdx_x - 2.0/3.0*divU_x);
   Pii_yy = -etaVis*(2.0*Vx/hy_cc  - 2.0/3.0*divU);
   Pii_zz = -etaVis_z*(2.0*dVzdz_z - 2.0/3.0*divU_z);
   Pii_zx = -etaVis_z*(dVxdz_z + dVzdx_z);
   Pii_xz = -etaVis_x*(dVxdz_x + dVzdx_x);
   
   /*
   // take divergence for x-momentum equation
   //
   Xgrid.DDX(divPii_x,Pii_xx*hy_ce);
   divPii_x /= hy_cc;
   Xgrid.DDZ(divUz,Pii_zx);
   divPii_x += divUz;
   if(geometry0=="CYL") divPii_x -= Pii_yy/hy_cc;
   Xgrid.communicate(divPii_x);
   
 
   // take divergence for z-momentum equation
   //
   Xgrid.DDX(divPii_z,Pii_xz*hy_ce);
   divPii_z /= hy_cc;
   Xgrid.DDZ(divUz,Pii_zz);
   divPii_z += divUz;
   ...
   // interpolate velocity to edges for source term
   // in energy equation
   //
   ...
   //
   UidotPii_x = Vx_x*Pii_xx + Vz_x*Pii_xz; 
   UidotPii_z = Vx_z*Pii_zx + Vz_z*Pii_zz; 
...
{ 
   ...
   Pii_xz.initialize(nXce,nZcc,0.0);
   Pii_zx.initialize(nXcc,nZce,0.0);
   ...
   etaVis.initialize(nXcc,nZcc,0.0);
   etaVis_x.initialize(nXce,nZcc,0.0); 
   etaVis_z.initialize(nXcc,nZce,0.0); 
...
void addMembersToDataFile( HDF5dataFile&  dataFile )
{
   ...
   dataFile.add(Pii_zx,"Pii_zx", 1); 
   dataFile.add(etaVis,"etaVis", 1); 
