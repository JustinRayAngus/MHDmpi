...
string advScheme0, TVDlimiter;   // advection differencing scheme
double K, cx, cz;
...
void computeFluxes(const domainGrid& Xgrid, const int order)
{
   ...
   if(advScheme0 == "TVD") {
      ...
      //                     FluxAdvCC_x+FluxDifCC_x,Cspeed,F0,TVDlimiter,0,order);
      //Xgrid.computeFluxTVD(Flux_z,FluxL_z,FluxR_z,FluxLimL_z,FluxLimR_z,
      //                     FluxAdvCC_z+FluxDifCC_z,Cspeed,F0,TVDlimiter,1,order);
   }      
   else {
