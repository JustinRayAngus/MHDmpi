...
%
subplot(2,3,2);
hold on; plot(Xcc,Mx(index0,index1,1),'black'); box on;
%hold on; plot(Xcc,Vx(index0,index1,11),'b');
hold on; plot(Xcc,Mx(index0,index1,end-plotBackIndex),'r'); grid on;
set(gca,'xtick',0:0.25:2);
%set(gca,'ytick',0:0.3:1.2);
xlabel('x'); ylabel('V');
...
%
fMx3 = -squeeze(Jzcc(3,:,2).*By(3,:,2));
%fMx3 = -squeeze(Jzcc(3,:,2));
fMx1 = zeros(size(fMx3));
for k=2:length(Xce)
    fMx1(k) = -(FluxMx_x(3,k,2)-FluxMx_x(3,k-1,2))/dX;
end
fMxtot = fMx1+fMx3;
% 
figure(10); hold on; 
plot(Xcc(3:end-2),fMxtot(3:end-2),'r--');
