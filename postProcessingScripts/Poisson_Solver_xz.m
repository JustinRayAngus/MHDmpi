...
for n = 1:mmax
    ...
    a = ones(nx,1); % - dx/2.0./xplot(:,1);
    b = -(2+km(n)^2*dx^2)*ones(nx,1);  % diagonal compoments
    c = ones(nx,1); % + dx/2.0./xplot(:,1);
